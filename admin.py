...
def _FetchWithCookie(self, url):
  ...
  try:
    ...
    logging.error(e)
  return urlfetch.fetch(url, headers={'cookie': 'SACSID=' + cookie})
...
class UmaQuery(Object):
  ...
  def _FetchData(self, date):
    ...
    return (json.loads(json_content)['r'], result.status_code)

  def _SaveData(self, data, date):
    property_map = self.property_map_class.get_all()

    for bucket_str, rate in data:
